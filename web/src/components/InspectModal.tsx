...
const FIELD = "w-full rounded-md border border-slate-300 px-3 py-1.5 font-mono text-sm text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100";
const MODE = "px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800";
    ...
    <Dialog open={isOpen()} onOpenChange={setIsOpen}>
      ...
      <Dialog.Portal>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
          <Dialog.Content class="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-5 shadow-xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <form class="mt-4 grid gap-3" onSubmit={event => void handleSubmit(event)}>
              ...
              <div class="flex justify-end gap-2">
                ...
                <button type="submit" disabled={submitState().phase === "running"} class="rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white">
