...
pub fn delta(
    ...
    base: Option<&str>,
    head: Option<&str>,
) -> Result<Vec<NewFinding>, ManifestError> {
    let ecosystem = ecosystem(path)?;
    let before = base
        .map(|text| parse(path, ecosystem, text))
        .transpose()?
        .unwrap_or_default();
    let after = head
        .map(|text| parse(path, ecosystem, text))
        ...
        .collect::<BTreeSet<_>>()
    {
        ...
        findings.push(NewFinding {
            ...
            detail: match movement {
                VersionMovement::Added => format!(
                    "{} was added with constraint {}{}.",
                    dependency.0,
                    dependency.1,
                    source_suffix(dependency.2.as_deref())
                ),
                VersionMovement::Removed => {
                    format!("{} was removed from the manifest.", dependency.0)
                }
                VersionMovement::Changed => format!(
                    "{} changed from the previous manifest declaration.",
                    dependency.0
                ),
                _ => unreachable!(),
            },
...
fn parse(
    ...
    match ecosystem {
        ...
        Ecosystem::Nix => Err(ManifestError::Unsupported {
            path: path.as_str().to_owned(),
        }),
    }
}
...
fn dependency_tables<'a>(
    tables: impl IntoIterator<Item = Option<&'a toml::Value>>,
) -> Vec<Dependency> {
    tables
        .into_iter()
        .filter_map(|table| table.and_then(toml::Value::as_table))
        .flat_map(|table| {
...
fn json_dependency_tables(value: &serde_json::Value, names: [&str; 4]) -> Vec<Dependency> {
    names
        .into_iter()
        .filter_map(|name| value.get(name).and_then(serde_json::Value::as_object))
        .flat_map(|table| {
            table.iter().map(|(name, value)| {
...
mod tests {
    ...
    fn reports_cargo_add_remove_and_constraint_changes() {
        ...
        assert!(
            findings
                .iter()
                .any(|finding| finding.movement == Some(VersionMovement::Added))
        );
        assert!(
            findings
                .iter()
                .any(|finding| finding.movement == Some(VersionMovement::Removed))
        );
        assert!(
            findings
                .iter()
                .any(|finding| finding.movement == Some(VersionMovement::Changed))
        );
    }
    ...
    fn reports_npm_source_changes_without_executing_manifest_content() {
        let findings = delta(
            &path("package.json"),
            Some(r#"{"dependencies":{"x":{"version":"1","resolved":"https://registry"}}}"#),
            Some(r#"{"dependencies":{"x":{"version":"1","resolved":"https://mirror"}}}"#),
        )
        .expect("parses");
    ...
    fn rejects_malformed_manifests() {
        assert!(matches!(
            delta(&path("Cargo.toml"), None, Some("[dependencies")),
    ...
    fn rejects_unsupported_manifest_paths() {
        assert!(matches!(
            delta(&path("README.txt"), None, Some("anything")),
