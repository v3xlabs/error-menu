...
/// `<use>`. Only a fragment pointing back into the same file is kept; anything else, a
/// value this does not understand included, counts as external.
fn has_external_reference(lower: &str) -> bool {
    ...
    while let Some(found) = lower[index..].find("href") {
        ...
        if !lower[cursor..cursor + end].trim_start().starts_with('#') {
            return true;
        }
        index = cursor + end;
    }
