...
impl FromStored for PersonRole {
    const FIELD: &'static str = "role";

    fn parse_stored(value: &str) -> Option<Self> {
        match value {
            "author" => Some(PersonRole::Author),
            "committer" => Some(PersonRole::Committer),
            "co_author" => Some(PersonRole::CoAuthor),
            "signed_off_by" => Some(PersonRole::SignedOffBy),
            "submitter" => Some(PersonRole::Submitter),
            "reviewer" => Some(PersonRole::Reviewer),
            _ => None,
        }
    }
}
